...
import { repo, freeze, integrationHashes, marketHashes } from './freeze.mjs';
...
async function command(arm, operation, task, profile, expected = 0, human = false) {
  ...
  })[operation]] : [join(repo, 'test/cross-application/control.mjs'), operation, task.root, task.id, profile ?? '-', task.app];
...
async function scenario(arm, appKey, kind) {
  ...
  const task = { id: `${arm}-${appKey}-${kind}`, root: join(root, `${arm}-${appKey}-${kind}`), app: join(repo, app.file) };
  const review = await provider(false);
  const domainFailure = kind === 'domain-failure', hookFailure = kind === 'native-hook';
  const rejectedAudit = ['after-run', 'invalid-publication'].includes(kind);
  try {
    ...
    await writeJson(join(task.root, 'web-input.local.json'), { app_key: appKey, mode: 'scripted',
      ...
      audit_fault: kind === 'after-run', invalid_audit: kind === 'invalid-publication' ? 'contract' : undefined,
    ...
    for (const session of snapshot.sessions) {
      ...
      for (const event of failures) {
        ...
        if (phase !== 'a') {
          assert.equal(event.payload.phase, hookFailure ? 'tool_execution_end' : 'after-run');
          assert.equal(event.payload.failure_class, hookFailure ? 'native-hook'
      ...
      if (arm === 'loom') assert.equal(session.events.at(-1).type, `session.${session.status}`);
...
try {
  for (const arm of smoke ? ['loom'] : ['loom', 'control']) for (const app of ['a', 'b', 'c'])
    for (const kind of phase === 'a' || smoke ? ['normal'] : ['normal', 'native-hook', 'after-run', 'invalid-publication', 'domain-failure']) {
