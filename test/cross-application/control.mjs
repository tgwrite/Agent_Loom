...
try {
  if (operation === 'create') {
    ...
    if (operation === 'inspect') {
      ...
      const aspectFailure = (pluginId, phase, failureClass) => record.events.push({ type: 'observer.failed',
        task_id: taskId, session_id: id, actor_id: record.actor.id, timestamp: new Date().toISOString(),
        payload: { contract_version: 2, plugin_id: pluginId, phase, failure_class: failureClass, failure: { code: 'NativeAspectFailed',
      ...
      try {
        ...
        await session.bindExtensions({ mode: 'rpc', onError(error) {
          ...
          if (aspect) aspectFailure(ids[index], ['session_start', 'session_shutdown', 'tool_execution_end'].includes(error.event) ? error.event : 'unknown', 'native-hook');
        } });
        ...
        for (const plugin of plugins.slice(1)) {
          ...
          try {
            const publications = await adapter.afterRun(session, context, outcome);
            failureClass = 'publication-validation';
            await publish(plugin, publications);
          } catch { aspectFailure(plugin.id, 'after-run', failureClass); }
        }
        if (outcome === 'failed') fail('NativeExecutionFailed');
