...
import { LocalTaskStore, inspectTask } from '../dist/packages/container-core/src/internal.js';
  ...
  try {
    for (let i = 0; i < count; i++) {
      ...
      artifacts.push({ id: `artifact-${i}`, task_id: 'task', type: 'sample', version: '1',
        producer: { session_id: id, plugin_id: 'domain', }, executor: { actor_id: 'actor', runtime_id: 'synthetic' },
        assertion: { status: 'READY' }, payload_ref: { kind: 'file', path: 'synthetic.json' }, sha256: 'a'.repeat(64), created_at: stamp });
