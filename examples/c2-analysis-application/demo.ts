...
try {
  const store = await LocalTaskStore.create(root, { schema_version: 1, id: 'demo-task',
    application_id: 'c2-analysis', title: 'Synthetic Artifact handoff', created_at: timestamp });
  ...
  await store.publishArtifact({ id: 'demo-handoff', task_id: store.task.id,
    type: 'DecoderHandoffView', version: '3',
