...
export interface PiHostModuleOptions {
  ...
  /** Optional explicit native preflight hook. Must not start a model or domain operation. */
  checkLauncher?(): Promise<void>;
}
...
export function createPiHostModule(options: PiHostModuleOptions) {
  ...
  const bindingsFor = (application: ApplicationDefinition, profileId?: string) => {
    ...
    if (application.runtime.id !== 'pi' || application.runtime.version !== options.sdkVersion)
      throw hostReadinessFailure('sdk-version');
  ...
  return {
    async validateNativeApplication({ application }: { application: ApplicationDefinition }): Promise<NativeReadinessReport> {
      ...
      for (const profile of application.profiles) {
        ...
        try {
          ...
          if (loaded.errors.length || loaded.extensions.length !== pending.size
            ...
            throw hostReadinessFailure('extension-loading');
        } catch (error) { throw safeHostReadinessFailure(error, 'extension-loading'); }
      }
      if (options.checkLauncher) {
        try { await options.checkLauncher(); } catch (error) { throw safeHostReadinessFailure(error, 'launcher'); }
      }
